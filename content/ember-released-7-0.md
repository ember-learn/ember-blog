...
In Ember 6.x minor releases, we landed many notable features (along with various bug fixes, of course):

- [In Ember 6.1](https://blog.emberjs.com/ember-released-6-1/) we began publishing `ember-source` as a v2 addon, which enabled `embroider` to consume `ember-source` directly.
- [In Ember 6.3](https://blog.emberjs.com/ember-released-6-3/) we added support for template-tag components as route templates per [RFC #1046](https://rfcs.emberjs.com/id/1046-template-tag-in-routes).
- [In Ember 6.8](https://blog.emberjs.com/ember-released-6-8/) we landed a new app blueprint with `Vite`(https://vite.dev/)-based `embroider` build system as the default.
- [Also in Ember 6.8](https://blog.emberjs.com/ember-released-6-8/) we moved to `strict-mode` (aka `template-tag`) component authoring format as the default.
- [Also in Ember 6.8](https://blog.emberjs.com/ember-released-6-8/) we added the [`renderComponent` API](https://rfcs.emberjs.com/id/1099-rendercomponent/) that allows rendering individual components without a full project. 
- [Also in Ember 6.8](https://blog.emberjs.com/ember-released-6-8/) we built in tracked versions of JavaScript's native collection types: `trackedArray`, `trackedObject`, `trackedMap`, `trackedSet`, `trackedWeakMap`, and `trackedWeakSet`.

Ember 6.8 was the biggest change to how an Ember project is built in recent memory. 
We flipped the defaults in v6.8 but both Vite-based embroider build-system and
...
- Did the same with `router.js` to prepare for experimentation with a new router architecture.
...
- Supported the Classic Blueprint (@ember-tooling/classic-build-app-blueprint) and supported it in [ember-cli-update](https://github.com/ember-cli/ember-cli-update) so that projects can update without moving to the new build system, if necessary. 
...
2. Upgrade your project to the latest patch version of Ember 6.12.
   ...
   `npx ember-cli-update --to 6.12`. The
...
6. Upgrade your app to Ember 7.0. Again,
   ...
   `npx ember-cli-update --to 7.0`. Refer to the
   [general Ember upgrade guide](https://cli.emberjs.com/release/basic-use/upgrading/#upgradinganemberappitself)
...
Ember.js 7.0 introduced 0 deprecations. It removed all deprecations that were
...
- [#21076](https://github.com/emberjs/ember.js/pull/21076) [BUGFIX] Support default globals for strict mode with the _runtime_ template compiler per [RFC #1070](https://rfcs.emberjs.com/id/1070-default-globals-for-strict-mode/).
- [#21098](https://github.com/emberjs/ember.js/pull/21098) [BUGFIX] Add support for `this` in explicit scope for the _runtime_ template compiler.
...
- [#21125](https://github.com/emberjs/ember.js/pull/21125) [BUGFIX] Fix crash when accessing negative index of helper positional args
...
Ember.js 7.0 introduced no new features.
